...
export function UserDetailCharts({ data }: { data: AdminUserDetailData }) {
  const categoryData = (Object.keys(data.byCategory) as TransactionCategory[])
    .map((cat) => ({
      category: cat,
      label: CATEGORY_LABELS[cat] || cat,
      volume: data.byCategory[cat]?.volume ?? 0,
      count: data.byCategory[cat]?.count ?? 0,
    }))
    .filter((d) => d.volume > 0 || d.count > 0)
    .sort((a, b) => b.volume - a.volume);

  const trendData = data.activityTrend;

  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      {/* 1. Daily Processing Velocity */}
      <div className="lg:col-span-2 rounded-2xl border border-hairline bg-canvas p-4 sm:p-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-medium text-ink">User Processing Velocity</h3>
            <p className="text-xs text-muted">Daily volume and count of transactions logged by this user</p>
          </div>
              ...
              <BarChart data={trendData} barGap={2} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  content={({ active, payload, label }) => {
                    ...
                    const item = payload[0].payload as { volume: number; count: number };
                    return (
                      <div className="rounded-xl border border-hairline bg-canvas px-3 py-2 shadow-lg">
                        ...
                        <p className="mt-0.5 text-xs text-muted">
                          {item.count.toLocaleString()} transaction{item.count === 1 ? "" : "s"}
                        </p>
                      </div>
                    );
                  }}
      ...
      <div className="rounded-2xl border border-hairline bg-canvas p-4 sm:p-5">
        ...
        <p className="text-xs text-muted">Distribution of payments logged</p>
